var zs_template_productlist = {
	...
	add_product : function(sId, sVal) {
		...
		zapjs.zw.api_call('com_cmall_productcenter_service_api_ApiGetSkus', {
			skuStrs : sVal
	...
	show_productlist : function() {
		...
		for ( var p=0,j=aObj.length;p<j;p++) {
			var othis = aObj[p];
			aHtml.push('<li>');
			aHtml.push('<div class="c_move">拖动</div>');
			...
			aHtml.push('<div class="c_image"><img src="' + othis["skuPicUrl"]
					+ '"/></div>');
			aHtml.push('<div class="c_name">' + othis["skuName"] + '</div>');
			...
			handle : '.c_move'
		}).droppable(
				{
					...
					onDrop : function(e, source) {
						...
						else if ((c.offset().left > $(this).offset().left)||(c.offset().top > $(this).offset().top)) {
							c.insertBefore(this);
						} else {
							c.insertAfter(this);
						}
	...
	save_productlist : function() {
		...
		var aHtml = [];
		aHtml.push('<div class="c_theme_productlist"><ul>');

		$('.zat_template_productlist_show li').each(
				function(n, el) {
					...
					aHtml.push('<div class="c_name">' + othis["skuName"]
							+ '</div>');
					aHtml.push('<div class="c_price">' + othis["sellPrice"]
							+ '</div>');
					aHtml.push('<div class="c_mark">' + othis["marketPrice"]
							+ '</div>');
					aHtml.push('<a target="_blank" herf="' + othis["skuCode"]
							+ '"><div class="c_buy"></div></a>');
					aHtml.push('<div>');
					aHtml.push('</li>');
				}
		...
		aHtml.push('</ul><div class="c_theme_clear"></div></div>');
